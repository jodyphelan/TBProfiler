...
def map(self):
	if self.params["platform"] == "Illumina":
		if self.mapper == "bwa":
			...
			if self.params["fq1"] and self.params["fq2"]:
				cmd = "%(bowtie2)s -p %(threads)s -x %(reffile)s --rg-id %(prefix)s --rg '@RG\\tID:%(prefix)s\\tSM:%(prefix)s\\tPL:%(platform)s' -1 %(fq1)s -2 %(fq2)s | samtools view -@ %(threads)s -b - | samtools sort -@ %(threads)s -o %(bamfile)s -" % self.params
			elif self.params["fq1"]:
				cmd = "%(bowtie2)s -p %(threads)s -x %(reffile)s --rg-id %(prefix)s --rg '@RG\\tID:%(prefix)s\\tSM:%(prefix)s\\tPL:%(platform)s' -U %(fq1)s | samtools view -@ %(threads)s -b - | samtools sort -@ %(threads)s -o %(bamfile)s -" % self.params
