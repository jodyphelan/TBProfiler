...
def calculate(args):
	...
	for d in list(results.keys()):
		...
		total_counts = counts[d]["tp"]+counts[d]["fn"]+counts[d]["tn"]+counts[d]["fp"]
		se_sensitivity = sensitivity*(1-sensitivity)/total_counts
		se_specificity = specificity*(1-specificity)/total_counts
		CI_sensitivity = 1.96*se_sensitivity
		CI_specificity = 1.96*se_specificity
		print("%s\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f\t%.5f" % (d,sensitivity,sensitivity-CI_sensitivity,sensitivity+CI_sensitivity,specificity,specificity-CI_specificity,specificity+CI_specificity))
